  ...
  <div class="favorites-view">
    <div class="favorites-content">
      ...
      <div v-if="currentFolder" class="folder-header">
        <div class="folder-info">
          ...
          <div class="folder-details">
            <h2 class="folder-name">{{ currentFolder.name }}</h2>
            <p class="folder-description" v-if="currentFolder.description">
              {{ currentFolder.description }}
            </p>
            <div class="folder-stats">
              <span class="stat">
                <span class="stat-value">{{ folderAudios.length }}</span>
                <span class="stat-label">个音频</span>
              </span>
      ...
      <div v-else class="all-favorites-header">
        <h2 class="header-title">全部收藏</h2>
        <p class="header-description">您在所有收藏夹中的音频文件</p>
        <div class="folder-stats">
          <span class="stat">
            <span class="stat-value">{{ folderAudios.length }}</span>
            <span class="stat-label">个音频</span>
          ...
          <span class="stat">
            <span class="stat-value">{{ userFavoriteFolders.length }}</span>
...
const currentFolder = computed(() => {
  ...
  return audioStore.getUserFavoriteFolders.find(folder => folder._id === currentFolderId.value)
})
...
// 计算属性：当前收藏夹的音频列表
const folderAudios = computed(() => {
  if (!currentFolderId.value) {
    // 全部收藏：显示所有收藏的音频
    return audioStore.favorites
  } else {
    // 特定收藏夹：显示该收藏夹的音频
    // 注意：这里需要根据实际数据结构调整过滤逻辑
    return audioStore.favorites.filter(audio => 
      audio.favorite && 
      audio.favorite.isFavorite && 
      audio.favorite.folder && 
      audio.favorite.folder._id === currentFolderId.value
    )
  }
})
...
const loadFolderData = async () => {
  try {
    ...
    // 如果当前是特定收藏夹视图，加载该收藏夹的音频
    if (currentFolderId.value) {
      const result = await audioStore.fetchFolderAudios(currentFolderId.value)
      // 这里可以根据需要处理返回的数据
      console.log('Loaded folder audios:', result)
    } else {
      // 加载全部收藏
